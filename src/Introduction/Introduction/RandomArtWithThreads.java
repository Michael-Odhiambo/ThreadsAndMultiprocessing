...
/**
 * A program that shows randomly generated "art". When the user clicks a "Start" button,
 * a new random artwork is generated every two seconds.
 * This program demonstrates using a thread for a very simple animation. The thread uses
 * Platform.runLater() to redraw the canvas on the javaFX application thread.
 */
public class RandomArtWithThreads extends Application {
    ...
    private volatile boolean running;  // Set to true while thread is started; It is set
                                       // to false as a signal to the thread that it should stop.
    private Runner runner;  // The thread that drives the animation.
    private Button startButton;  // A button that is used to start and stop the animation.
    ...
    /**
     * If called when an animation is running, this method fills the canvas with a random
     * work of "art". If no thread is running, it just fills the canvas with white.
     */
    private void redraw() {
