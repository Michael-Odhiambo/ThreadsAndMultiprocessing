...
/**
 * This demo program uses several threads to compute an image "in the background". It is a modification of the
 * BackgroundComputationDemo, which used only one thread. As rows of pixels in the image are computed, they are
 * copied to the screen. ( The image is a small piece of the famous Mandelbrot set, which is used just because it
 * takes some time to compute. There is no need to understand what the image means.) The user starts the computation
 * by clicking a "Start" button. A pop-up menu allows the user to select the number of threads to be used. The
 * specified number of threads is created and each thread is assigned a region in the image. The threads are run at
 * lower priority, which will make sure that the GUI thread will get a chance to run to repaint the display as
 * necessary.
 */

public class MultipleProcessingDemo1 extends Application {
    ...
    private volatile int threadsRunning;  // Keeps track of the number of threads that are running.
    ...
    private Color[] palette;  // The color palette, containing the colors of the spectrm.
     ...
     * screen; otherwise, size will be reduced to fit. This method also makes the color palette, containing colors
     * in spectral order.
     */
    public void start( Stage stage ) {
        ...
        stage.setTitle( "MultiProcessing Demonstration 1.0" );
        stage.show();

    }

    /**
     * This method is called from the computation threads when one row of pixels needs to be added to the image.
    ...
    private void drawOneRow( int rowNumber, Color[] colorArray ) {
        for ( int i = 0; i < width; i++ ) {
            // Color an individual pixel by filling in a 1-by-1 pixel rectangle. Not the most
            // efficient way to do this, but good enough for this demo.
    ...
    /**
     * This method is called when the user clicks the button. If no computation is currently running, it starts as many
     * new threads as the user has specified, and assigns a different part of the image to each thread. The threads are
     * run at lower priority that the event-handling thread, in order to keep the GUI responsive. If a computation is in
     * progress when this method is called, running is set to false as a signal to stop all the threads.
     */
    private void doStartOrStop() {
        if ( running ) {
            startButton.setDisable( true ); // Will be re-enabled when all threads have stopped. ( Prevent user from
                                            // from trying to stop threads that are already stopping.)
     ...
     * the name of the button to "Start Again" and re-enabling the pop-up menu.
     */
