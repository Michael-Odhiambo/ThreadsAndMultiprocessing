...
/**
 * This demo program divides up a large computation into a fairly large number of smaller tasks. The computation is to
 * compute an image, and each task computes one row of pixels in the image. A thread pool is created at the beginning of
 * the program, with one thread for each available processor. The threads remove tasks from a blocking queue and execute
 * them. The threads never terminate ( until the program ends ). To start a computation, tasks are created and added to
 * the blocking queue. As soon as the first tasks are added to the queue, the threads "wake up" and start working on
 * them.
 * ( The image is a small piece of the famous Mandelbrot set, which is used just because it takes time to compute. There
 * is no need to understand what the image means. )
 */
...
public class MultiProcessingDemo3 extends Application {
    ...
    /**
     * Job number of the current computation job. A "job" is the computation of an entire image, whereas a "task" is one
     * part of a job. jobNumber is incremented after a job has completed. Note that any left-over tasks from a job are
     * ignored when they finally complete. This can happen when the user aborts a computation, since if a thread is
     * working on a task when that happens, it will continue to work on that task until the task completes, so the task
     * can complete after the job of which it is a part has been aborted. ( Note: The task itself could check the
     * current job number as it is running and terminate early if the job number has changed. This would make sense if
     * the task's computation were very long. )
     */
    private int jobNumber;

    /**
     * How many tasks in the current job still remain to be done? ( REMARK: jobInProgress, jobNumber, and tasksRemaining
     * can all be modified by more than one thread. They do not need to be volatile because all access to these
     * variables is done in synchronized methods. )
     */
    private int tasksRemaining;

    private Button startButton;  // Button the user can click to start or abort the thread.
    private Canvas canvas;  // Canvas where the image is displayed.
    ...
    /**
     * Set up the GUI and event handling. The canvas will be 1200 - by- 1000 pixels, if that fits comfortably on the
     * screen; otherwise, size will be reduced to fit. This method also makes the color palette, containing colors in
     * spectral order.
     */
    ...
    /**
     * This method is called from the computation threads when one row of pixels need to be added to the image.
    ...
    private void drawOneRow( int rowNumber, Color[] colorArray ) {
        for ( int i = 0; i < width; i++ ) {
            // Color an individual pixel by filling in a 1-by-1 pixel rectangle. Not
            // the most efficient way to do this, but good enough for this demo.
    ...
    /**
     * This method is called the first time the user clicks "Start" to create the thread pool and the blocking queue of
     * tasks that is used to send tasks to the thread pool. The number of threads is equal to the number of available
     * processors. There is no need to keep any references to the thread objects. They only have to be started here and
     * they will just keep running until the program ends.
     */
    private void createThreadPool() {
        taskQueue = new LinkedBlockingQueue<>();
        int processors = Runtime.getRuntime().availableProcessors();
        for ( int i = 0; i < processors; i++ ) {
            // Threads will block while waiting for tasks to arrive in the queue.
    ...
    /**
     * This method is called when the user clicks the Start button. If no computation is in progress, it clears the
     * image and sets up the computation of a new image. The first time that it is called, it is also responsible for
    ...
    /**
     * This method is called by each thread when it terminates. We keep track of the number of threads that have
     * terminated, so that when they have all finished, we can put the program into the correct state, such as changing
     * the name of the button to "Start Again". This method is responsible for drawing the row of pixels computed by the
     * task to the canvas. It only does that if the task is part of the current job, not a previous, aborted job.
     */
