 ...
 * compute an image, and each task computes one row of pixels in the image. The tasks are placed into a thread_safe
 * queue. Several "worker" threads remove tasks from the queue and carry them out. When all the tasks have completed,
 * the worker threads terminate. The number of worker threads is specified by the user. ( The image is a small piece of
 * the famous Mandelbrot set, which is used just because it takes some time to compute. There is no need to understand
...
public class MultiProcessingDemo2 extends Application {
    ...
    private ComboBox<String> threadCountSelector;  // For specifying the number of threads to be used.
     ...
     * otherwise, size will be reduced to fit. This method also makes the color palette, containing colors in spectral
     * order.
     */
    public void start( Stage stage ) {
        ...
        threadCountSelector.getItems().addAll( "Use 1 thread.", "Use 2 threads.", "Use three threads.",
                "Use 4 threads.", "Use 5 threads.", "Use 6 threads.", "Use 7 threads.", "Use 8 threads.",
                "Use 9 threads.", "Use 10 threads.", "Use 20 threads." );
        threadCountSelector.getSelectionModel().select( 1 );

        HBox bottom = new HBox( 8, startButton, threadCountSelector );
    ...
    private void drawOneRow( int rowNumber, Color[] colorArray ) {
        for ( int i = 0; i < width; i++ ) {
            // Color an individual pixel by filling in a 1-by-1 pixel rectangle. Not the most efficient way to do this
    ...
    private void doStartOrStop() {
        if (running) {
            ...
            threadCountSelector.setDisable(true); // will be re-enabled when all threads finish
            ...
            int threadCount = threadCountSelector.getSelectionModel().getSelectedIndex() + 1;
            if (threadCount == 11)
                threadCount = 20;
    ...
    /**
     * This class defines the worker threads that carry out the tasks. A WorkerThread runs in a loop in which it
     * retrieves a task from the taskQueue and calls the run() method in that task. The thread terminates when the queue
     * is empty. ( Note that for this to work properly, all the tasks must be placed into the queue before the thread is
     * started. If the queue is empty when the thread starts, the thread will simply exit immediately. ) The thread also
     * terminates if the signal variable, running, is set to false. Just before it terminates, the thread calls the
