...
/**
 * This class is part of a demonstration of distributed computing. It is to be used with CLMandelbrotTask.java on the
 * master computer. Before starting CLMandelbrotMaster on the master computer on the master computer, CLMandelbrotWorker
 * should be started as a command-line program on each worker computer involved in the distributed computation. See
 * CLMandelbrotWorker.java for more information. ( It is possible to run several copies of CLMandelbrotWorker on the
 * same computer, as long as each copy uses a different port number. It is even possible to run the master and one or
 * more copies of the worker on the same computer. In that case, the host name for the workers would be local host. ) If
 * CLMandelbrotMaster is run with no command line arguments, it will run without using the network at all. The complete
 * computation will be done locally ( using just one processor ). You can do this to test how long the computation takes
 * whe distributed computing is not used. Otherwise, the location of all the CLMandelbrotWorker programs must be
 * specified as command line arguments when CLMandelbrotMaster is run. Each command line argument gives the host name or
 * IP address of a computer on which a worker is running. If that worker is not listening on the default port, then the
 * listening port number must also be included. The form for specifying a computer with a port number is, for example
 * michael.allan.edu:1501 or 127.0.0.1:18881, that is, the computer name or IP, followed by a colon, followed by the
 * port number, with NO SPACES.
 *
 * When CLMandelbrotMaster runs, it creates a list of tasks ( of type CLMandelbrotTask ) that have to be performed, and
 * it creates a thread for communicating with each copy of CLMandelbrotWorker. Each thread sends a sequence of tasks to
 * the connected worker, which does the actual work involved in performing the task and sends back the results.
 * Although this program is meant as a demonstration of distributed computing, it does compute an interesting picture.
 *
 * Note that data sent over the network is encoded as text. The first word on a line of text identifies the type of
 * data.
 */
public class CLMandelbrotMaster {
     ...
     * ( This features is currently unused. )
     */
     ...
     * by the incoming data ( id, maxIterations, y, xmin, dx, and count ) for the task. Items on the line are separated
     * by spaces.
     */
    private static final String TASK_COMMAND = "task";

    /**
     * The first word on a message representing the results from a CLMandelbrotTask. This is followed by the task id,
     * the number of items in the results, and then the results. Items on the line are separated by spaces.
     */
    private static final String RESULT_COMMAND = "result";

    /**
     * The list of tasks that must be performed to complete the computation. This list is created by createJob(). The
     * tasks are sent out to workers to be performed. Each task represents the computation of one row of an image of
     * part of the Mandelbrot set.
     */
    private static ConcurrentLinkedQueue<CLMandelbrotTask> tasks;

    /**
     * The number of tasks that have been completed. At the end of the computation, this should be equal to the number
     * of rows in the image that is being computed.
     */
    ...
    /**
     * Maximum number of iterations for the Mandelbrot computation; set by computeJob(). For the purposes of this
     * computation, it is not necessary to understand this.
     */
