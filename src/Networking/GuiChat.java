...
/**
 * Opens a window that can be used for a two-way network chat. The window can "listen" for a connection request on a
 * port that is specified by the user. It can request a connection to another GuiChat window on a specified computer and
 * port. The window has an input box where the user can enter messages to be sent over the connection. A connection can
 * be closed by clicking a button in the window or by closing the window. To test the program, several copies of the
 * program can be run on the same computer.
 */
...
public class GuiChat extends Application {
    ...
    /**
     * Default port number. This is the initial content of input boxes in the window that specify the port number for
     * the connection.
     */
    private static String defaultPort = "1501";

    /**
     * Default host name. This is the initial content of the input box that specifies the name of the computer to which
     * a connection request will be sent.
     */
    ...
    /**
     * Input box for messages that will be sent to the other side of the network connection.
     */
    private TextField messageInput;

    /**
     * Contains a transcript of messages sent and received, along with information about the progress and stage of the
     * connection.
     */
    ...
    public void start( Stage stage ) {
        ...
        HBox connectBar = new HBox( 5, listenButton, listeningPortInput, connectButton, remoteHostInput,
                new Label( "Port:"), remotePortInput );
    ...
    /**
     * Defines responses to buttons. ( In this program, I use one method to handle all the buttons; the source of the
     * event can be used to determine which button was clicked. )
     */
    ...
    /**
     * Defines the thread that handles the connection. The thread is responsible for opening the connection and for
     * receiving messages. This class contains several methods that are called by the main class, and that are therefore
     * executed in a different thread. Note that by using a thread to open the connection, any blocking of the graphical
     * user interface is avoided. By using a thread for reading messages sent from the other side, the messages can be
     * received and posted to the transcript asynchronously at the same time as the user is typing and sending messages.
     * All changes to the GUI that are made by this class are done using Platform.runLater().
     */
    private class ConnectionHandler extends Thread {
        ...
        /**
         * Listen for a connection on a specified port. The constructor does not perform any network operations; it just
         * sets some instance variables and starts the thread. Note that the thread will only listen for one connection,
         * and then will close its server socket.
         */
        ...
        /**
         * Open a connection to specified computer and port. The constructer does not perform any network operations; it
         * just sets some instance variables and starts the thread.
         */
        ...
        /**
         * Send a message to the other side of the connection, and post the message to the transcript. This should only
         * be called when the connection state is ConnectionState.CONNECTED; if it is called at other times, it is
         * ignored. ( Although it is unlikely, it is possible for this method to block, if the system's buffer for
         * outgoing data fills.
         */
        ...
        /**
         * Close the connection. If the server socket is non-null, the server socket is closed, which will cause its
         * accept() method to fail with an error. If the socket is non-null, then the socket is closed, which will cause
         * its input method to fail with an error. ( However, these errors will not be reported to the user. )
         */
        ...
        /**
         * This is called by the run() method when a message is received from the other side of the connection. The
         * message is posted to the transcript, but only if the connection state is CONNECTED. ( This is because a
         * message might be received after the user has clicked the "Disconnect" button; that message should not be seen
         * by the user. )
         */
        ...
        /**
         * This is called by the run() method when the connection has been successfully opened. It enables the correct
         * buttons, writes a message to the transcript, and sets the connected state to CONNECTED.
         */
        ...
        /**
         * This is called by the run() method when the connection is closed from the other side. ( This is detected when
         * an end-of-stream is encountered on the inputstrea. ) It posts a message to the transcript and sets connection
         * state to CLOSED.
         */
        ...
        /**
         * Called from the finally clause of the run() method to clean up after the network connection closes for any
         * reason.
         */
        ...
        /**
         * The run() method that is executed by the thread. it opens a connection as a client or as a server ( depending
        ...
        public void run() {
            try {
                ...
                connectionOpened();  // Set up to use connection.
                ...
                // An error occurred. Report it to the user, but not if the connection has been closed ( since the error
                // might be expected error that is generated when a socket is closed ).
